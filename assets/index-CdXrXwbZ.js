import{f as G}from"./firebase-CNzVcu9p.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=a(s);fetch(s.href,r)}})();const we={apiKey:"AIzaSyBIlCbxhxEucKdBNyMWEwRuFOpEo0dVQx8",authDomain:"equilibrioprodutivo-app.firebaseapp.com",databaseURL:"https://equilibrioprodutivo-app-default-rtdb.firebaseio.com",projectId:"equilibrioprodutivo-app",storageBucket:"equilibrioprodutivo-app.firebasestorage.app",messagingSenderId:"785521986199",appId:"1:785521986199:web:449d7fe7979e0aad8db7d2",measurementId:"G-2FZ69XYWZJ"};G.apps.length||G.initializeApp(we);const ke="equilibrio_produtivo_data",be={daily_logs:{},finances:{transactions:[],balance:0},learning:[],kanban:{ideas:[],doing:[],done:[]},settings:{accent_color:"#72fe8f"}};function Le(){const e=localStorage.getItem(ke);if(!e)return JSON.parse(JSON.stringify(be));try{const t=JSON.parse(e);return{daily_logs:t.daily_logs||{},finances:t.finances||{transactions:[],balance:0},learning:Array.isArray(t.learning)?t.learning:[],kanban:t.kanban||{ideas:[],doing:[],done:[]},settings:t.settings||{accent_color:"#72fe8f"}}}catch{return JSON.parse(JSON.stringify(be))}}const _e=(e=new Date)=>{const t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0");return`${t}-${a}-${n}`},oe=()=>_e(new Date),d={_uid:null,init:async e=>{d._uid=e;try{if(!(await G.database().ref(`users/${e}`).once("value")).exists()){const a=Le();await G.database().ref(`users/${e}`).set(a)}}catch(t){console.error("[DB] Erro de inicialização",t)}},getRef:e=>{if(!d._uid)throw new Error("Usuário não autenticado");return G.database().ref(`users/${d._uid}/${e}`)},getSettings:async()=>{const e=await d.getRef("settings").once("value");return e.exists()?e.val():{accent_color:"#72fe8f"}},saveSettings:async e=>{const a={...await d.getSettings(),...e};return await d.getRef("settings").set(a),a},getHabits:async()=>{const e=await d.getRef("settings/habits").once("value");return e.exists()?e.val():null},saveHabits:async e=>{await d.getRef("settings/habits").set(e)},getTodayLog:async()=>{const e=oe(),t=await d.getRef(`daily_logs/${e}`).once("value");if(!t.exists()){const a={habits:{},mood:null,sleep:null,water:0,screen_time:0,rest_day:!1};return await d.getRef(`daily_logs/${e}`).set(a),a}return t.val()},updateHabit:async(e,t,a=null)=>{const n=a||oe();await d.getRef(`daily_logs/${n}/habits`).update({[e]:t})},updateDailyMetrics:async(e,t,a=null)=>{const n=a||oe();await d.getRef(`daily_logs/${n}`).update({[e]:t})},updateDailyFinances:async(e,t)=>{const a={income_dia:Number((t==null?void 0:t.income_dia)||0),expense_dia:Number((t==null?void 0:t.expense_dia)||0),income_din:Number((t==null?void 0:t.income_din)||0),expense_din:Number((t==null?void 0:t.expense_din)||0)};await d.getRef(`daily_logs/${e}`).update(a)},getMonthlyLogs:async e=>{const t=await d.getRef("daily_logs").orderByKey().startAt(e).endAt(e+"").once("value");return t.exists()?t.val():{}},getDailyLog:async e=>{const t=await d.getRef(`daily_logs/${e}`).once("value");return t.exists()?t.val():null},getAllDailyLogs:async()=>{const e=await d.getRef("daily_logs").once("value");return e.exists()?e.val():{}},getFinances:async()=>{const e=await d.getRef("finances").once("value");if(!e.exists())return{transactions:[],balance:0};const t=e.val();return{transactions:t.transactions||[],balance:t.balance||0}},getEmergencyFund:async()=>{const e=await d.getRef("finances/emergency_fund").once("value");return e.exists()?Number(e.val()):0},saveEmergencyFund:async e=>{await d.getRef("finances/emergency_fund").set(Number(e))},addTransaction:async e=>{e.id=Date.now().toString();const t=await d.getFinances();return t.transactions||(t.transactions=[]),t.transactions.push(e),e.type==="income"?t.balance+=Number(e.amount):t.balance-=Number(e.amount),await d.getRef("finances").set(t),e},getLibrary:async()=>{const e=await d.getRef("learning").once("value");return e.exists()?e.val():[]},saveLibraryItem:async e=>{let t=await d.getLibrary();Array.isArray(t)||(t=[]);const a=t.findIndex(n=>n.id===e.id);return a>=0?t[a]=e:t.push(e),await d.getRef("learning").set(t),e},deleteLibraryItem:async e=>{let t=await d.getLibrary();t=(t||[]).filter(a=>a.id!==e),await d.getRef("learning").set(t)},getKanbanData:async()=>{const e=await d.getRef("kanban").once("value");if(!e.exists())return{ideas:[],doing:[],done:[]};const t=e.val();return{ideas:t.ideas||[],doing:t.doing||[],done:t.done||[]}},saveKanbanData:async e=>{await d.getRef("kanban").set(e)}};function $e({todayLog:e,balances:t,todayPct:a,missing:n,isAllDone:s,weekData:r,snapWeeks:o=[],currentWeekIndex:c=0,DEFAULT_HABITS:f,snapMessage:k,libraryItems:m}){const v=l=>Number(l||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}),p=[{card:"bg-gradient-to-br from-cyan-500/12 via-surface-container-highest to-blue-500/10",border:"border-cyan-300/25",glow:"shadow-[0_8px_22px_rgba(34,211,238,0.14)]",bar:"bg-cyan-300 shadow-[0_0_10px_rgba(103,232,249,0.45)]",pct:"text-cyan-300",stepper:"bg-cyan-400/10 border-cyan-300/25 text-cyan-200 hover:bg-cyan-400/20"},{card:"bg-gradient-to-br from-emerald-500/12 via-surface-container-highest to-lime-500/10",border:"border-emerald-300/25",glow:"shadow-[0_8px_22px_rgba(52,211,153,0.14)]",bar:"bg-emerald-300 shadow-[0_0_10px_rgba(110,231,183,0.45)]",pct:"text-emerald-300",stepper:"bg-emerald-400/10 border-emerald-300/25 text-emerald-200 hover:bg-emerald-400/20"},{card:"bg-gradient-to-br from-amber-500/12 via-surface-container-highest to-orange-500/10",border:"border-amber-300/25",glow:"shadow-[0_8px_22px_rgba(251,191,36,0.14)]",bar:"bg-amber-300 shadow-[0_0_10px_rgba(252,211,77,0.45)]",pct:"text-amber-300",stepper:"bg-amber-400/10 border-amber-300/25 text-amber-200 hover:bg-amber-400/20"},{card:"bg-gradient-to-br from-fuchsia-500/12 via-surface-container-highest to-pink-500/10",border:"border-fuchsia-300/25",glow:"shadow-[0_8px_22px_rgba(217,70,239,0.14)]",bar:"bg-fuchsia-300 shadow-[0_0_10px_rgba(240,171,252,0.45)]",pct:"text-fuchsia-300",stepper:"bg-fuchsia-400/10 border-fuchsia-300/25 text-fuchsia-200 hover:bg-fuchsia-400/20"},{card:"bg-gradient-to-br from-indigo-500/12 via-surface-container-highest to-violet-500/10",border:"border-indigo-300/25",glow:"shadow-[0_8px_22px_rgba(99,102,241,0.14)]",bar:"bg-indigo-300 shadow-[0_0_10px_rgba(165,180,252,0.45)]",pct:"text-indigo-300",stepper:"bg-indigo-400/10 border-indigo-300/25 text-indigo-200 hover:bg-indigo-400/20"}],w=l=>{const L=`${(l==null?void 0:l.id)||""}${(l==null?void 0:l.title)||""}${(l==null?void 0:l.type)||""}`;let i=0;for(let j=0;j<L.length;j++)i=(i<<5)-i+L.charCodeAt(j),i|=0;const g=Math.abs(i)%p.length;return p[g]},T=l=>{const L=l.total>0?Math.round(l.current/l.total*100):0,i=l.type==="book",g=w(l),j={to_do:"Para Iniciar",in_progress:"Em Andamento",done:"Concluído"},$={to_do:"text-on-surface-variant/60 bg-white/5 border-white/10",in_progress:"text-blue-400 bg-blue-400/10 border-blue-400/20",done:"text-green-400 bg-green-400/10 border-green-400/20"},E=i?"Pág":"Aula",S=$[l.status]||$.to_do,y=l.status==="in_progress",C=l.status==="done"?"opacity-60":"",D=y?"ring-2 ring-blue-400/40":"";return`
            <div class="min-w-[255px] rounded-3xl p-5 border space-y-5 flex flex-col relative cursor-pointer active:scale-95 transition-transform ${g.card} ${g.border} ${g.glow} ${D} ${C}" onclick="window.openLibraryView('${l.id}')">
                ...
                <div class="space-y-3 mt-auto">
                    <div class="h-1.5 w-full bg-surface-container rounded-full overflow-hidden border border-white/10">
                        <div class="h-full ${g.bar} rounded-full" style="width:${L}%"></div>
                    </div>
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-[10px] font-extrabold ${g.pct}">${L}%</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <button class="w-8 h-8 rounded-xl border text-base font-extrabold transition-colors ${g.stepper}" onclick="event.stopPropagation(); window.adjustLibraryProgress('${l.id}', -1)">-</button>
                        <button class="w-8 h-8 rounded-xl border text-base font-extrabold transition-colors ${g.stepper}" onclick="event.stopPropagation(); window.adjustLibraryProgress('${l.id}', 1)">+</button>
            ...
            </div>`},x={in_progress:0,to_do:1,done:2},B=l=>[...l].sort((L,i)=>(x[L.status]??1)-(x[i.status]??1)),I=B((m||[]).filter(l=>l.type==="course")),F=B((m||[]).filter(l=>l.type==="book")),A=l=>`<div class="min-w-[240px] bg-surface-container rounded-3xl p-5 border border-dashed border-white/10 flex items-center justify-center"><span class="text-sm text-on-surface-variant/30">Nenhum ${l} cadastrado</span></div>`,H=(l,L,i,g,j=!1)=>{if(i==="outside")return`
            ...
            </div>`;const $=100.53,E=$-$*g/100,S=g===100;let y=j?`<span class="material-symbols-outlined text-black opacity-90" style="font-size: 18px; font-variation-settings: 'FILL' 1;">hotel</span>`:S?`<span class="material-symbols-outlined text-black opacity-90" style="font-size: 20px; font-variation-settings: 'FILL' 1;">local_fire_department</span>`:`<span class="text-[9px] font-extrabold tracking-tight text-on-surface-variant">${g}%</span>`;const _=i==="today"?'id="snap-ring-today-circle"':"",C=i==="today"?'id="snap-ring-today-text"':"";let D="";return j?D='<circle cx="20" cy="20" r="16" fill="#fbbf24" stroke="transparent" />':S?D='<circle cx="20" cy="20" r="16" fill="var(--accent-color)" stroke="transparent" class="accent-bg" />':D=`
        ...
        <div class="flex flex-col items-center gap-1.5 flex-shrink-0 min-w-[40px]">
            ...
            <div class="relative w-10 h-10 flex items-center justify-center rounded-full" ${j?'style="box-shadow: 0 0 15px rgba(251,191,36,0.45);"':S?'style="box-shadow: 0 0 15px var(--accent-color);"':""} ${i==="today"?'id="snap-ring-today-container"':""}>
                ...
                <div class="z-10 flex items-center justify-center" ${C}>${y}</div>
            </div>
        </div>`},P=o&&o.length>0?o:[{index:0,label:"Semana 1",rangeLabel:"",days:r||[]}],O=P.map((l,L)=>`<span data-week-dot="${L}" class="w-1.5 h-1.5 rounded-full ${L===c?"bg-primary accent-bg":"bg-white/20"}"></span>`).join(""),K=P.map((l,L)=>{const i=l.days.filter(g=>g.state!=="outside"&&g.pct===100&&!g.isRestDay).length;return`
            <div data-week-index="${L}" class="min-w-full shrink-0 snap-start">
                ...
                <div class="flex flex-wrap justify-between gap-y-6 gap-x-2 w-full pb-2 px-1">
                    ${l.days.map(g=>H(g.day,g.dayNumber,g.state,g.pct,g.isRestDay)).join("")}
                </div>
            </div>
        `}).join("");return`
        <div class="space-y-8 pb-12">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-amber-400/5 rounded-3xl p-6 border border-amber-300/20 opacity-90">
                        ...
                        <div class="flex items-baseline gap-1">
                            ...
                            <span class="block w-full max-w-full truncate text-[clamp(1.1rem,4.8vw,1.85rem)] font-extrabold tracking-tighter text-[var(--text-primary)] font-headline" title="R$ ${v(t==null?void 0:t.diaBalance)}">${v(t==null?void 0:t.diaBalance)}</span>
                        </div>
                    </div>
                    <div class="bg-emerald-400/10 rounded-3xl p-6 border border-emerald-300/25 opacity-90">
                        ...
                        <div class="flex items-baseline gap-1">
                            ...
                            <span class="block w-full max-w-full truncate text-[clamp(1.1rem,4.8vw,1.85rem)] font-extrabold tracking-tighter text-[var(--text-primary)] font-headline" title="R$ ${v(t==null?void 0:t.dinheiroBalance)}">${v(t==null?void 0:t.dinheiroBalance)}</span>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="space-y-4">
                ...
                <div class="flex gap-4 overflow-x-auto hide-scrollbar -mx-6 px-6 pb-4" style="scrollbar-width: none; -ms-overflow-style: none;">
                    ${F.length>0?F.map(T).join(""):A("livro")}
                </div>
            </section>
            ...
            <section class="space-y-4">
                ...
                <div class="flex gap-4 overflow-x-auto hide-scrollbar -mx-6 px-6 pb-4" style="scrollbar-width: none; -ms-overflow-style: none;">
                    ${I.length>0?I.map(T).join(""):A("curso")}
                </div>
            </section>
        </div>
        ...
        <div id="library-modal" class="fixed inset-0 z-[200] hidden flex-col justify-end">
            ...
            <div class="relative w-full h-[95vh] bg-surface-container-low rounded-t-[40px] flex flex-col shadow-[0_-20px_40px_rgba(0,0,0,0.5)] transform translate-y-full transition-transform duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)]" id="library-modal-sheet">
                ...
                <div class="flex-1 overflow-y-auto px-6 py-6 pb-32 space-y-4 hide-scrollbar" id="library-modal-list">
                    ${(()=>{const l=m||[];if(l.length===0)return'<p class="text-center text-on-surface-variant/30 text-sm py-12">Nenhuma obra cadastrada ainda.</p>';const L=(y,_=!1)=>{const C=y.total>0?Math.round(y.current/y.total*100):0,D=y.type==="book",M=w(y),R={to_do:"Para Iniciar",in_progress:"Em Andamento",done:"Concluído"},u={to_do:"text-on-surface-variant/60 bg-white/5 border-white/10",in_progress:"text-blue-400 bg-blue-400/10 border-blue-400/20",done:"text-green-400 bg-green-400/10 border-green-400/20"},h=D?"Pág":"Aula",b=D?"Livro":"Curso",N=u[y.status]||u.to_do,q=y.status==="in_progress"?"ring-2 ring-blue-400/30":"",z=_?"opacity-60":"";return`
                            <div class="w-full rounded-[28px] p-5 border space-y-5 flex flex-col relative cursor-pointer active:scale-[0.98] transition-transform ${M.card} ${M.border} ${M.glow} ${q} ${z}" data-lib-type="${y.type}" onclick="window.openLibraryView('${y.id}')">
                                <div class="flex justify-between items-start">
                                    <span class="text-3xl filter drop-shadow-md">${y.emoji||(D?"📘":"🎓")}</span>
                                    <span class="text-[10px] font-bold ${N} px-3 py-1.5 rounded-xl uppercase tracking-widest border">${R[y.status]||"Para Iniciar"}</span>
                                </div>
                                <div>
                                    <span class="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest mb-1 block">${b} • ${y.author||""}</span>
                                    <h4 class="font-bold text-[var(--text-primary)] text-[18px] leading-tight">${y.title}</h4>
                                </div>
                                <div class="space-y-3 mt-auto">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <button class="text-xs font-bold text-on-surface-variant hover:text-white transition-colors" onclick="event.stopPropagation(); window.quickSetLibraryProgress('${y.id}')">${h} ${y.current||0} de ${y.total||0}</button>
                                        <span class="text-xs font-extrabold ${M.pct}">${C}%</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button class="w-9 h-9 rounded-xl border text-lg font-extrabold transition-colors ${M.stepper}" onclick="event.stopPropagation(); window.adjustLibraryProgress('${y.id}', -1)">-</button>
                                        <button class="w-9 h-9 rounded-xl border text-lg font-extrabold transition-colors ${M.stepper}" onclick="event.stopPropagation(); window.adjustLibraryProgress('${y.id}', 1)">+</button>
                                        <button class="flex-1 h-9 rounded-xl border border-white/10 bg-black/20 text-[10px] font-bold uppercase tracking-widest text-white/80 hover:bg-white/10 transition-colors" onclick="event.stopPropagation(); window.quickSetLibraryProgress('${y.id}')">Definir atual</button>
                                    </div>
                                </div>
                            </div>`},i={in_progress:0,to_do:1,done:2},g=[...l].sort((y,_)=>(i[y.status]??1)-(i[_.status]??1)),j=g.filter(y=>y.status!=="done"),$=g.filter(y=>y.status==="done"),E=j.map(y=>L(y,!1)).join(""),S=$.length>0?`
                            <div class="pt-2">
    ...
    `}const De=[{id:"wakeup_early",name:"Acordar cedo",icon:"wb_sunny"},{id:"gym",name:"Academia",icon:"fitness_center"},{id:"breakfast",name:"Café da manhã",icon:"coffee"},{id:"lunch",name:"Almoço",icon:"restaurant"},{id:"study_dio",name:"Estudos DIO",icon:"school"},{id:"reading",name:"Leitura",icon:"menu_book"},{id:"dinner",name:"Janta",icon:"restaurant_menu"},{id:"fill_notion",name:"Preencher Notion",icon:"edit_note"}],ne=()=>window.APP_HABITS||De;function Ie(e,t){const a=Object.values(e||{}),n=a.reduce((r,o)=>r+Number(o.income_dia||0)-Number(o.expense_dia||0),0),s=a.reduce((r,o)=>r+Number(o.income_din||0)-Number(o.expense_din||0),0);return{diaBalance:n,dinheiroBalance:s}}function xe(e){if(!e)return 0;if(e.rest_day)return 100;const t=ne();let a=0;const n=e.habits||{};for(const s of t)n[s.id]&&a++;return Math.round(a/t.length*100)}function le(e=new Date){const t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0");return`${t}-${a}-${n}`}function Ee(e){const t=new Date(e),a=t.getDay(),n=a===0?-6:-(a-1);return t.setDate(t.getDate()+n),t.setHours(0,0,0,0),t}async function W(){var t;const e=document.getElementById("dashboard-root");try{const a=await d.getTodayLog(),n=await d.getLibrary();window._libraryItems=n;const s=ne();let r=0;a.habits||(a.habits={});for(const _ of s)a.habits[_.id]&&r++;const o=!!a.rest_day,c=xe(a),f=s.length-r,k=o||r===s.length,m=["D","S","T","Q","Q","S","S"],v=new Date,p=le(v),w=await d.getAllDailyLogs(),T=Ie(w,p),x=v.getMonth(),B=v.getFullYear(),I=`${B}-${String(x+1).padStart(2,"0")}`,F=new Date(B,x,1),A=new Date(B,x+1,0),H=[];let P=0,O=!1,K=0,l=Ee(F);for(;l<=A;){const _=[];let C=!1;for(let D=0;D<5;D++){const M=new Date(l);M.setDate(l.getDate()+D);const u=M.getMonth()===x,h=le(M);u&&(C=!0);const b=u?w==null?void 0:w[h]:null,N=u?xe(b):0,V=!!(b&&b.rest_day),q=u?h===p?"today":h<p?"past":"future":"outside";_.push({day:m[M.getDay()],dayNumber:M.getDate(),dateKey:h,state:q,pct:N,isRestDay:V}),h===p&&(P=K,O=!0)}if(C){const D=_.find(u=>u.state!=="outside"),R=[..._].reverse().find(u=>u.state!=="outside");H.push({index:K,label:`Semana ${K+1}`,rangeLabel:D&&R?`${String(D.dayNumber).padStart(2,"0")} - ${String(R.dayNumber).padStart(2,"0")}`:"",days:_}),K++}l.setDate(l.getDate()+7)}if(!O&&H.length>0){const _=H.findLastIndex(C=>C.days.some(D=>D.state!=="outside"&&D.dateKey<=p));P=_>=0?_:0}let L=P;window._snapWeekMonthKey===I&&Number.isInteger(window._snapWeekIndex)&&(L=Math.max(0,Math.min(window._snapWeekIndex,H.length-1))),window._snapWeekMonthKey=I;const i=((t=H[L])==null?void 0:t.days)||[],g=i.filter(_=>_.pct===100&&!_.isRestDay).length,j=v.getDay()===1,$=i.findIndex(_=>_.state==="today");let E=`${g} dias perfeitos. <span class="text-primary accent-text">Não quebre a sequência hoje!</span>`;o&&(E="Hoje é dia de descanso. Recuperar também é disciplina."),g===5?E="Semana Lendária concluída! Descanse nos fins de semana.":j&&c===0?E="Tela em branco. Vamos desenhar uma semana perfeita?":$>0&&i[$-1].pct<100?E="Ontem foi dia de descanso, mas hoje é foco total!":c===0&&(E="O dia está voando. Hora do primeiro check-in!"),e.innerHTML=$e({todayLog:a,balances:T,todayPct:c,missing:f,isAllDone:k,weekData:i,snapWeeks:H,currentWeekIndex:L,DEFAULT_HABITS:ne(),snapMessage:E,libraryItems:n});const S=document.getElementById("snap-weeks-carousel"),y=S==null?void 0:S.querySelector(`[data-week-index="${L}"]`);if(S&&y){requestAnimationFrame(()=>{S.scrollTo({left:y.offsetLeft,behavior:"auto"})});const _=()=>{const D=Array.from(S.querySelectorAll("[data-week-index]")),M=Array.from(document.querySelectorAll("[data-week-dot]"));if(!D.length||!M.length)return;let R=0,u=Number.POSITIVE_INFINITY;for(const h of D){const b=Number(h.getAttribute("data-week-index")),N=Math.abs(h.offsetLeft-S.scrollLeft);N<u&&(u=N,R=b)}window._snapWeekIndex=R,M.forEach(h=>{const b=Number(h.getAttribute("data-week-dot"))===R;h.classList.toggle("bg-primary",b),h.classList.toggle("accent-bg",b),h.classList.toggle("bg-white/20",!b)})};let C=null;S.addEventListener("scroll",()=>{C||(C=requestAnimationFrame(()=>{_(),C=null}))},{passive:!0}),_()}await Q()}catch(a){console.error(a),e.innerHTML=`<div style="color:red; padding:20px; word-break:break-all;"><h3>Erro no Dashboard:</h3><pre>${a.message}
${a.stack}</pre></div>`}}window.openCheckinModal=async()=>{const e=document.getElementById("checkin-modal"),t=document.getElementById("checkin-modal-overlay"),a=document.getElementById("checkin-modal-sheet");e.classList.remove("hidden"),e.classList.add("flex");const n=new Date,s=document.getElementById("lbl-checkin-date"),r=document.getElementById("lbl-checkin-day");s&&(s.textContent=n.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"}).replace(".","").toUpperCase()),r&&(r.textContent=n.toLocaleDateString("pt-BR",{weekday:"long"}));const o=await d.getTodayLog(),c={nervoso:"Nervoso",feliz:"Feliz",produtivo:"Produtivo",normal:"Normal",ansioso:"Ansioso",cansado:"Cansado",triste:"Triste"};if(o.mood){const v=c[o.mood];document.querySelectorAll(".mood-btn").forEach(p=>{p.textContent.trim()===v&&window.selectChip(p,"mood-btn",!0)})}const f={perfeito:"Perfeito",muito_bom:"Muito bom",bom:"Bom",mais_ou_menos:"Mais ou menos",ruim:"Ruim"};if(o.sleep){const v=f[o.sleep];document.querySelectorAll(".sleep-btn").forEach(p=>{p.textContent.trim()===v&&window.selectChip(p,"sleep-btn",!0)})}const k=o.water||0;if(k>0)for(let v=1;v<=5;v++){const p=document.getElementById(`water-drop-${v}`);p&&(v<=k?(p.classList.remove("grayscale","opacity-30"),p.classList.add("drop-shadow-[0_0_15px_rgba(34,211,238,0.6)]","filter-none")):(p.classList.add("grayscale","opacity-30"),p.classList.remove("drop-shadow-[0_0_15px_rgba(34,211,238,0.6)]","filter-none")))}const m=document.getElementById("input-wake-time");m&&(m.value=o.wake_time||""),window.toggleRestDay(!!o.rest_day,!0),requestAnimationFrame(()=>{t.classList.remove("opacity-0"),a.classList.remove("translate-y-full")})};window.closeCheckinModal=async()=>{const e=document.getElementById("input-wake-time"),t=[];e&&t.push(d.updateDailyMetrics("wake_time",e.value||""));const a=document.getElementById("rest-day-toggle-checkin"),n=(a==null?void 0:a.dataset.active)==="true";t.push(d.updateDailyMetrics("rest_day",!!n));const s=document.getElementById("input-fluxo-dia-income"),r=document.getElementById("input-fluxo-dia-expense"),o=document.getElementById("input-fluxo-din-income"),c=document.getElementById("input-fluxo-din-expense");if(s||r||o||c){const v={income_dia:s&&parseFloat(s.value)||0,expense_dia:r&&parseFloat(r.value)||0,income_din:o&&parseFloat(o.value)||0,expense_din:c&&parseFloat(c.value)||0},p=le(new Date);t.push(d.updateDailyFinances(p,v))}await Promise.all(t);const f=document.getElementById("checkin-modal"),k=document.getElementById("checkin-modal-overlay"),m=document.getElementById("checkin-modal-sheet");k.classList.add("opacity-0"),m.classList.add("translate-y-full"),setTimeout(()=>{f.classList.add("hidden"),f.classList.remove("flex"),W()},500)};let fe=null;window.openLibraryView=e=>{const a=(window._libraryItems||[]).find(T=>T.id===e)||{id:e,emoji:"📘",title:"",author:"",type:"book",status:"",current:0,total:0};fe=a,document.getElementById("lbl-lv-emoji").innerText=a.emoji||"📘",document.getElementById("lbl-lv-title").innerText=a.title||"",document.getElementById("lbl-lv-author").innerText=a.author||"";const n=a.total>0?Math.round(a.current/a.total*100):0,s=a.type==="book",r=s?"bg-cyan-400 shadow-[0_0_10px_rgba(136,235,255,0.5)]":"bg-primary accent-bg shadow-[0_0_10px_rgba(var(--accent-color-rgb),0.4)]",o=s?"text-cyan-400":"text-primary accent-text",c=s?"Pág":"Aula",f={to_do:"Para Iniciar",in_progress:"Em Andamento",done:"Concluído"},k={to_do:"text-on-surface-variant/60 border-white/10",in_progress:"text-blue-400 border-blue-400/20 bg-blue-400/10",done:"text-green-400 border-green-400/20 bg-green-400/10"},m=[];m.push(`
        ...
        <div class="space-y-6 pb-12 font-headline animate-[fade-in_0.4s_ease-out]">
            ...
            <section class="bg-surface-container-low rounded-[40px] p-6 border border-white/5 shadow-2xl relative overflow-hidden">
                ...
                <div class="flex items-center justify-between mb-5 px-1">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="window.nextCalMonth()" class="w-9 h-9 rounded-xl border border-white/10 flex items-center justify-center transition-colors active:scale-90 ${v?"bg-surface-highest/30 text-on-surface-variant/30 cursor-not-allowed":"bg-surface-highest text-on-surface-variant hover:text-primary"}" ${v?"disabled":""}>
            ...
            <section class="space-y-4 pt-6">
                 ...
                 <div class="flex items-start gap-4 overflow-x-auto hide-scrollbar -mx-6 px-6 pb-6 pt-2 snap-x" style="scrollbar-width: none;">
                    ${(()=>{const i={estudo:"text-blue-400 bg-blue-400/10 border-blue-400/20",hobbie:"text-purple-400 bg-purple-400/10 border-purple-400/20",crescimento:"text-green-400 bg-green-400/10 border-green-400/20",trabalho:"text-orange-400 bg-orange-400/10 border-orange-400/20",saude:"text-cyan-400 bg-cyan-400/10 border-cyan-400/20",outro:"text-on-surface-variant bg-white/5 border-white/10"},g=$=>{const E=i[$.type]||i.outro;return`
                    <div class="kanban-card flex-none p-4 bg-surface-container rounded-2xl shadow-[0_8px_25px_rgba(0,0,0,0.3)] border border-white/5 cursor-pointer hover:bg-surface-highest hover:border-white/10 transition-all relative overflow-hidden group" draggable="true" data-card-id="${$.id}" onclick="window.openKanbanView('${$.id}')" ondragstart="event.stopPropagation()" >
                        ...
                        <div class="flex-shrink-0 self-start w-[290px] flex flex-col gap-3">
                            <h4 class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant pl-4 flex items-center gap-2">
                                ...
                                ${E} <span class="w-1 h-1 bg-white/20 rounded-full"></span> <span class="text-primary">${y.length}</span>
                            </h4>
                            <div class="kanban-column h-auto bg-surface-container-low/50 backdrop-blur rounded-[32px] p-4 min-h-[350px] border border-white/5 space-y-3 shadow-inner" data-column="${$}">
                                ${y.map(_=>g(_)).join("")}
                                ${y.length===0?'<div class="kanban-empty-state h-full flex items-center justify-center opacity-20 flex-col gap-2 mt-20"><span class="material-symbols-outlined text-4xl">inbox</span><p class="text-[10px] uppercase font-bold tracking-widest">Nada por aqui</p></div>':""}
                            </div>
                        </div>`;return`
        ...
        <div id="full-history-modal" class="fixed inset-0 z-[400] hidden flex-col">
            <div class="relative w-full h-full bg-surface-container-low flex flex-col pt-10 transform scale-95 opacity-0 transition-all duration-300" id="full-history-sheet">
                ...
                <div class="px-8 py-4 flex gap-3 overflow-x-auto hide-scrollbar">
                    ${(k||[]).map(i=>`<button
                            class="history-month-btn px-5 py-2 rounded-2xl border font-bold text-xs transition-colors ${i.key===m?"bg-primary/20 text-primary border-primary/30":"bg-white/5 text-on-surface-variant border-transparent hover:bg-white/10"}"
    ...
    `}let X=new Date().getFullYear(),J=new Date().getMonth();window.prevCalMonth=()=>{J--,J<0&&(J=11,X--),U()};window.nextCalMonth=()=>{const e=new Date;(X<e.getFullYear()||X===e.getFullYear()&&J<e.getMonth())&&(J++,J>11&&(J=0,X++),U())};const Se=[{id:"wakeup_early",name:"Acordar cedo"},{id:"gym",name:"Academia"},{id:"breakfast",name:"Café da manhã"},{id:"lunch",name:"Almoço"},{id:"study_dio",name:"Estudos DIO"},{id:"reading",name:"Leitura"},{id:"dinner",name:"Janta"},{id:"fill_notion",name:"Preencher Notion"}],Y=()=>window.APP_HABITS||Se,ie=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"],ce=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];function ae(e){if(!e)return 0;if(e.rest_day)return 100;const t=Y();let a=0;const n=e.habits||{};for(const s of t)n[s.id]&&a++;return Math.round(a/t.length*100)}async function U(){const e=document.getElementById("planner-root");try{let t=await d.getKanbanData();window._kanbanAllCards=[...(t.ideas||[]).map(u=>({...u,progress:"ideas"})),...(t.doing||[]).map(u=>({...u,progress:"doing"})),...(t.done||[]).map(u=>({...u,progress:"done"}))];const a=new Date,n=X,s=J,r=`${n}-${String(s+1).padStart(2,"0")}`,o=new Date(n,s+1,0).getDate(),c=n===a.getFullYear()&&s===a.getMonth(),f=c?a.getDate():o,k=new Date(n,s,1).getDay(),m=await d.getMonthlyLogs(r),v=await d.getAllDailyLogs(),p=[];for(let u=1;u<=o;u++){const h=`${r}-${String(u).padStart(2,"0")}`,b=m[h],N=ae(b),V=!!b,q=!!(b&&b.rest_day);let z=0;q?z=4:N>0&&N<=33?z=1:N>33&&N<=66?z=2:N>66&&(z=3),p.push({day:u,level:z,pct:N,isFuture:u>f,isRestDay:q,rawDate:h,hasLog:V})}const w=[];for(let u=1;u<=o;u++){const h=`${r}-${String(u).padStart(2,"0")}`,b=m[h];w.push({day:u,rawDate:h,hasLog:!!b,habits:b&&b.habits?b.habits:{}})}const T=Object.entries(v||{}).sort((u,h)=>h[0].localeCompare(u[0])).map(([u,h])=>{const[b,N,V]=u.split("-").map(Number),q=ae(h),z=Y().map(re=>({id:re.id,name:re.name,done:!!(h.habits&&h.habits[re.id])}));return{date:`${String(V).padStart(2,"0")} ${ce[(N||1)-1]}`,rawDate:u,monthKey:`${b}-${String(N).padStart(2,"0")}`,pct:q,mood:h.mood||null,sleep:h.sleep||null,water:h.water||0,wake_time:h.wake_time||"",telas:h.screen_time||0,income_dia:h.income_dia||0,expense_dia:h.expense_dia||0,income_din:h.income_din||0,expense_din:h.expense_din||0,restDay:!!h.rest_day,habits:z}}),x=[],B={};for(const u of T)B[u.monthKey]=(B[u.monthKey]||0)+1;Object.keys(B).sort((u,h)=>h.localeCompare(u)).forEach(u=>{const[h,b]=u.split("-").map(Number);x.push({key:u,label:`${ie[(b||1)-1]} ${h}`,count:B[u]})});const I=r;window._plannerFullHistoryCurrentMonthKey=I;const F=[],A=a.getMonth(),P=`${a.getFullYear()}-${String(A+1).padStart(2,"0")}`,O=c?m:await d.getMonthlyLogs(P),K=c?o:a.getDate();for(let u=a.getDate();u>=1;u--){const h=`${P}-${String(u).padStart(2,"0")}`,b=O[h],N=b?ae(b):0,V=Y().map(q=>{var z;return{id:q.id,name:q.name,done:!!((z=b==null?void 0:b.habits)!=null&&z[q.id])}});F.push({date:`${String(u).padStart(2,"0")} ${ce[A]}`,rawDate:h,pct:N,mood:(b==null?void 0:b.mood)||null,sleep:(b==null?void 0:b.sleep)||null,water:(b==null?void 0:b.water)||0,wake_time:(b==null?void 0:b.wake_time)||"",telas:(b==null?void 0:b.screen_time)||0,income_dia:(b==null?void 0:b.income_dia)||0,expense_dia:(b==null?void 0:b.expense_dia)||0,income_din:(b==null?void 0:b.income_din)||0,expense_din:(b==null?void 0:b.expense_din)||0,restDay:!!(b!=null&&b.rest_day),habits:V})}window._plannerHistory=T,window._plannerHabitFilter={days:w,monthLabel:`${ie[s]} ${n}`,firstDayOffset:k,todayDate:f},window._plannerHabitFilterCurrentHabit||(window._plannerHabitFilterCurrentHabit="gym");const l=Object.values(m),L=l.filter(u=>ae(u)===100&&!u.rest_day).length,i={perfeito:5,muito_bom:4,bom:3,mais_ou_menos:2,ruim:1},g={5:"perfeito",4:"muito_bom",3:"bom",2:"mais_ou_menos",1:"ruim"},j={feliz:5,produtivo:4,normal:3,cansado:2,triste:1},$={5:"feliz",4:"produtivo",3:"normal",2:"cansado",1:"triste"};let E=0,S=0,y=0,_=0;for(const u of l)u.sleep&&i[u.sleep]&&(E+=i[u.sleep],S++),u.mood&&j[u.mood]&&(y+=j[u.mood],_++);const C=S>0?g[Math.round(E/S)]||"bom":"—",D=_>0?$[Math.round(y/_)]||"normal":"—",M=l.reduce((u,h)=>(u.totalGastoDia+=Number(h.expense_dia||0),u.totalGastoDinheiro+=Number(h.expense_din||0),u),{totalGastoDia:0,totalGastoDinheiro:0}),R={perfectDays:L,avgSleep:C,avgMood:D,totalGastoDia:M.totalGastoDia,totalGastoDinheiro:M.totalGastoDinheiro};e.innerHTML=Be({calendarData:p,calendarYear:n,calendarMonth:s,isCurrentMonth:c,historyDays:F,metrics:R,kanbanData:t,habitCatalog:Y(),habitFilterMonthLabel:`${ie[s]} ${n}`,fullHistoryRows:T,fullHistoryMonths:x,fullHistoryCurrentMonthKey:I}),Me()}catch(t){console.error("Planner error:",t),e.innerHTML=`<div style="color:red; padding:20px; word-break:break-all;"><h3>Erro no Planner:</h3><pre>${t.message}
${t.stack}</pre></div>`}}function me(){var v;const e=window._plannerHabitFilter;if(!e)return;const t=Y(),a=window._plannerHabitFilterCurrentHabit||((v=t[0])==null?void 0:v.id)||"gym",n=t.find(p=>p.id===a)||t[0],s=document.getElementById("habit-filter-grid"),r=document.getElementById("habit-filter-summary"),o=document.getElementById("habit-filter-selected-title");if(!s||!r||!o)return;document.querySelectorAll(".habit-filter-chip").forEach(p=>{const w=p.dataset.habit===n.id;p.classList.toggle("border-primary",w),p.classList.toggle("bg-primary/20",w),p.classList.toggle("text-primary",w),p.classList.toggle("border-white/10",!w),p.classList.toggle("bg-surface-highest",!w),p.classList.toggle("text-on-surface-variant",!w)});const c=e.days.filter(p=>{var w;return p.hasLog&&!!((w=p.habits)!=null&&w[n.id])}).length,f=e.days.filter(p=>p.hasLog).length;o.textContent=n.name,r.textContent=`${c} de ${f} dias com registro concluíram este hábito.`;const k=Array.from({length:e.firstDayOffset},()=>'<div class="aspect-square w-full"></div>').join(""),m=e.days.map(p=>{var H;const w=p.hasLog&&!!((H=p.habits)!=null&&H[n.id]),T=p.day===e.todayDate,x=p.hasLog?`onclick="window.openDailyDetail('${p.rawDate}')"`:"",B=w?"bg-primary/90 border border-primary/60 text-white shadow-[0_0_10px_var(--accent-color)]":p.hasLog?"bg-surface-highest border border-white/10 text-on-surface-variant":"bg-white/[0.04] border border-transparent text-white/20",I=T?"ring-2 ring-primary/70":"",F=p.hasLog?"cursor-pointer hover:scale-105 hover:brightness-110":"cursor-default",A=w?`<span class="absolute top-1 right-1 material-symbols-outlined text-[10px] text-black/70" style="font-variation-settings: 'FILL' 1;">check_circle</span>`:"";return`<div ${x} class="aspect-square w-full rounded-xl relative flex items-center justify-center text-[10px] font-extrabold transition-all ${B} ${I} ${F}">${p.day}${A}</div>`}).join("");s.innerHTML=k+m}window.openHabitFilterModal=()=>{var s;const e=document.getElementById("habit-filter-modal"),t=document.getElementById("habit-filter-overlay"),a=document.getElementById("habit-filter-sheet");if(!e||!t||!a)return;const n=document.getElementById("habit-filter-month-label");n&&((s=window._plannerHabitFilter)!=null&&s.monthLabel)&&(n.textContent=window._plannerHabitFilter.monthLabel),e.classList.remove("hidden"),e.classList.add("flex"),requestAnimationFrame(()=>{t.classList.remove("opacity-0"),a.classList.remove("translate-y-full")}),me()};window.setHabitCalendarFilter=e=>{window._plannerHabitFilterCurrentHabit=e,me()};window.closeHabitFilterModal=()=>{const e=document.getElementById("habit-filter-modal"),t=document.getElementById("habit-filter-overlay"),a=document.getElementById("habit-filter-sheet");!e||!t||!a||(t.classList.add("opacity-0"),a.classList.add("translate-y-full"),setTimeout(()=>{e.classList.add("hidden"),e.classList.remove("flex")},400))};window.openDailyDetail=(e,t=!1)=>{let a=window._plannerHistory.find(x=>x.date===e||x.rawDate===e);if(!a){const x=e.split("-").map(Number);if(x.length!==3)return;const[B,I,F]=x;a={date:`${String(F).padStart(2,"0")} ${ce[(I||1)-1]}`,rawDate:e,pct:0,mood:null,sleep:null,water:0,wake_time:"",telas:0,income_dia:0,expense_dia:0,income_din:0,expense_din:0,restDay:!1,habits:Y().map(A=>({id:A.id,name:A.name,done:!1}))},t=!0}const n=a.date||e,s=a.rawDate||e,r=document.getElementById("day-detail-modal"),o=document.getElementById("day-detail-overlay"),c=document.getElementById("day-detail-sheet"),f=document.getElementById("day-detail-content");document.getElementById("lbl-day-title").innerText=n,document.getElementById("lbl-day-pct").innerText=a.restDay?"Dia de Descanso":`${a.pct}% Concluído`;const k={nervoso:"Nervoso",feliz:"Feliz",produtivo:"Produtivo",normal:"Normal",ansioso:"Ansioso",cansado:"Cansado",triste:"Triste"},m={nervoso:"border-red-500 bg-red-500/20 text-red-500",feliz:"border-green-400 bg-green-400/20 text-green-400 shadow-[0_0_15px_rgba(74,222,128,0.3)]",produtivo:"border-cyan-400 bg-cyan-400/20 text-cyan-400",normal:"border-white/50 bg-white/10 text-white",ansioso:"border-orange-400 bg-orange-400/20 text-orange-400",cansado:"border-purple-400 bg-purple-400/20 text-purple-400",triste:"border-blue-400 bg-blue-400/20 text-blue-400"},v={perfeito:"Perfeito",muito_bom:"Muito bom",bom:"Bom",mais_ou_menos:"Mais ou menos",ruim:"Ruim"},p={perfeito:"border-purple-400 bg-purple-400/20 text-purple-400 shadow-[0_0_15px_rgba(192,132,252,0.3)]",muito_bom:"border-blue-400 bg-blue-400/20 text-blue-400",bom:"border-cyan-400 bg-cyan-400/20 text-cyan-400",mais_ou_menos:"border-orange-400 bg-orange-400/20 text-orange-400",ruim:"border-red-500 bg-red-500/20 text-red-500"},w=Math.round(a.water),T=[1,2,3,4,5].map(x=>t?`<button onclick="window.setWaterForDate('${s}', ${x})" class="text-4xl transition-all duration-300 ${x<=w?"drop-shadow-[0_0_15px_rgba(34,211,238,0.6)] filter-none":"grayscale opacity-30"} hover:scale-110 active:scale-90">💧</button>`:`<span class="text-4xl transition-all duration-300 ${x<=w?"drop-shadow-[0_0_15px_rgba(34,211,238,0.6)]":"grayscale opacity-30"}">💧</span>`).join("");f.innerHTML=`
        <section class="space-y-4">
        ...
        <section class="space-y-4">
            ...
            <div class="bg-surface-container-highest rounded-3xl p-5 border border-white/5 space-y-6">
                ...
                <div class="space-y-3">
                    <span class="text-sm font-bold text-[var(--text-primary)] block">Humor Geral</span>
                    <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-1 -mx-2 px-2" style="scrollbar-width:none;">
                        ${Object.keys(k).map(x=>{const B=x===a.mood;return t?`<button onclick="window.setQualitativeForDate('${s}', 'mood', '${x}')" class="flex-shrink-0 px-5 py-2.5 rounded-2xl border text-sm font-bold transition-all ${B?m[x]+" opacity-100":"border-transparent bg-surface-highest text-on-surface-variant opacity-30 hover:opacity-100"}">${k[x]}</button>`:`<span class="flex-shrink-0 px-5 py-2.5 rounded-2xl border text-sm font-bold transition-all ${B?m[x]:"border-transparent bg-surface-highest text-on-surface-variant opacity-30"}">${k[x]}</span>`}).join("")}
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <span class="text-sm font-bold text-[var(--text-primary)] block">Qualidade do Sono</span>
                    <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-1 -mx-2 px-2" style="scrollbar-width:none;">
                        ${Object.keys(v).map(x=>{const B=x===a.sleep;return t?`<button onclick="window.setQualitativeForDate('${s}', 'sleep', '${x}')" class="flex-shrink-0 px-5 py-2.5 rounded-2xl border text-sm font-bold transition-all ${B?p[x]+" opacity-100":"border-transparent bg-surface-highest text-on-surface-variant opacity-30 hover:opacity-100"}">${v[x]}</button>`:`<span class="flex-shrink-0 px-5 py-2.5 rounded-2xl border text-sm font-bold transition-all ${B?p[x]:"border-transparent bg-surface-highest text-on-surface-variant opacity-30"}">${v[x]}</span>`}).join("")}
                    </div>
                </div>
            </div>
        </section>
    ...
    `,r.classList.contains("hidden")&&(r.classList.remove("hidden"),r.classList.add("flex"),requestAnimationFrame(()=>{o.classList.remove("opacity-0"),c.classList.remove("translate-y-full")}))};window.toggleHabitForDate=async(e,t,a)=>{var s;const n=window._plannerHistory.find(r=>r.rawDate===e);if(n!=null&&n.restDay){(s=window.showToast)==null||s.call(window,"Dia de descanso ativo. Desative para editar hábitos.","info");return}if(await d.updateHabit(t,a,e),n){const r=n.habits.find(c=>c.id===t);r&&(r.done=a);let o=n.habits.filter(c=>c.done).length;n.pct=Math.round(o/n.habits.length*100)}window.openDailyDetail(e,!0)};window.setQualitativeForDate=async(e,t,a)=>{await d.updateDailyMetrics(t,a,e);const n=window._plannerHistory.find(s=>s.rawDate===e);n&&(n[t]=a),window.openDailyDetail(e,!0)};window.setWaterForDate=async(e,t)=>{await d.updateDailyMetrics("water",t,e);const a=window._plannerHistory.find(n=>n.rawDate===e);a&&(a.water=t),window.openDailyDetail(e,!0)};window.setRestDayForDate=async(e,t)=>{await d.updateDailyMetrics("rest_day",t,e);const a=window._plannerHistory.find(n=>n.rawDate===e);a&&(a.restDay=t,a.pct=t?100:Math.round(a.habits.filter(n=>n.done).length/a.habits.length*100)),window.openDailyDetail(e,!0)};window.saveAndCloseDailyDetail=async e=>{const t=[],a=document.getElementById("input-planner-wake-time");a&&t.push(d.updateDailyMetrics("wake_time",a.value||"",e));const n=document.getElementById("input-planner-dia-income"),s=document.getElementById("input-planner-dia-expense"),r=document.getElementById("input-planner-din-income"),o=document.getElementById("input-planner-din-expense");if(n||s||r||o){const c={income_dia:n&&parseFloat(n.value)||0,expense_dia:s&&parseFloat(s.value)||0,income_din:r&&parseFloat(r.value)||0,expense_din:o&&parseFloat(o.value)||0};t.push(d.updateDailyFinances(e,c))}await Promise.all(t),window.closeDailyDetail(),setTimeout(()=>{U()},550)};window.closeDailyDetail=()=>{const e=document.getElementById("day-detail-modal"),t=document.getElementById("day-detail-overlay"),a=document.getElementById("day-detail-sheet");t.classList.add("opacity-0"),a.classList.add("translate-y-full"),setTimeout(()=>{e.classList.add("hidden"),e.classList.remove("flex")},500)};window.filterFullHistoryMonth=e=>{document.querySelectorAll(".history-month-btn").forEach(n=>{const s=n.dataset.monthKey===e;n.classList.toggle("bg-primary/20",s),n.classList.toggle("text-primary",s),n.classList.toggle("border-primary/30",s),n.classList.toggle("bg-white/5",!s),n.classList.toggle("text-on-surface-variant",!s)});let t=0;document.querySelectorAll(".history-day-row").forEach(n=>{const s=n.dataset.monthKey===e;n.classList.toggle("hidden",!s),s&&t++});const a=document.getElementById("full-history-empty");a&&a.classList.toggle("hidden",t>0)};window.openFullHistory=()=>{var n;const e=document.getElementById("full-history-modal"),t=document.getElementById("full-history-sheet");e.classList.remove("hidden"),e.classList.add("flex"),requestAnimationFrame(()=>{t.classList.remove("scale-95","opacity-0")});const a=window._plannerFullHistoryCurrentMonthKey||((n=document.querySelector(".history-month-btn"))==null?void 0:n.getAttribute("data-month-key"));a&&window.filterFullHistoryMonth(a)};window.closeFullHistory=()=>{const e=document.getElementById("full-history-modal");document.getElementById("full-history-sheet").classList.add("scale-95","opacity-0"),setTimeout(()=>{e.classList.add("hidden"),e.classList.remove("flex")},300)};const je={estudo:"📚 Estudo",hobbie:"🎮 Hobbie",crescimento:"🌱 Crescimento",trabalho:"💼 Trabalho",saude:"🏋️ Saúde",outro:"📌 Outro"},Fe={ideas:"A Iniciar",doing:"Em Progresso",done:"Feito ✅"},Te={ideas:"text-on-surface-variant/60",doing:"text-blue-400",done:"text-green-400"};let te=null,Z=null;window.openKanbanView=e=>{var f,k;const t=document.querySelector(`.kanban-card[data-card-id="${e}"]`),a=(window._kanbanAllCards||[]).find(m=>m.id===e)||{id:e,emoji:((f=t==null?void 0:t.querySelector("span.text-2xl"))==null?void 0:f.innerText)||"🎯",title:((k=t==null?void 0:t.querySelector("p"))==null?void 0:k.innerText)||"",type:"",objective:"",description:"",start:"",end:""};Z=a,document.getElementById("lbl-kv-emoji").innerText=a.emoji||"🎯",document.getElementById("lbl-kv-title").innerText=a.title||"",document.getElementById("lbl-kv-type").innerText=je[a.type]||"";const n=[];a.objective&&n.push(`
        ...
        <div class="bg-surface-container-highest rounded-3xl px-5 py-4 border border-white/5 flex items-center justify-between">
            <span class="text-[10px] uppercase font-bold text-on-surface-variant/50 tracking-widest">Progresso</span>
            <span class="font-bold text-sm ${s}">${Fe[a.progress]||"A Iniciar"}</span>
    ...
    `}function Pe(){const e=document.createElement("div");e.id="fin-modals-host",e.innerHTML=`
    ...
    `,document.body.appendChild(e)}function ge(e){const t=document.getElementById(e);t.classList.remove("hidden"),t.classList.add("flex"),requestAnimationFrame(()=>{document.getElementById(`${e}-overlay`).classList.remove("opacity-0"),document.getElementById(`${e}-sheet`).classList.remove("translate-y-full")})}function ve(e){document.getElementById(`${e}-overlay`).classList.add("opacity-0"),document.getElementById(`${e}-sheet`).classList.add("translate-y-full"),setTimeout(()=>{const t=document.getElementById(e);t.classList.add("hidden"),t.classList.remove("flex")},500)}function Ne(e){const t=e==="dia",a=document.getElementById("fin-detail-title"),n=document.getElementById("fin-detail-icon"),s=document.getElementById("fin-detail-content");t?(a.textContent="Dia a Dia",n.className="w-9 h-9 rounded-xl bg-blue-500/15 flex items-center justify-center flex-none",n.innerHTML='<span class="material-symbols-outlined text-blue-400" style="font-size:18px">wallet</span>'):(a.textContent="Meu Dinheiro",n.className="w-9 h-9 rounded-xl bg-violet-500/15 flex items-center justify-center flex-none",n.innerHTML='<span class="material-symbols-outlined text-violet-400" style="font-size:18px">savings</span>');const r=t?"income_dia":"income_din",o=t?"expense_dia":"expense_din",c=Object.entries(de).filter(([,I])=>Number(I[r]||0)>0||Number(I[o]||0)>0).sort((I,F)=>F[0].localeCompare(I[0]));if(c.length===0){s.innerHTML=`
        ...
        `;return}const f=c.reduce((I,[,F])=>I+Number(F[r]||0),0),k=c.reduce((I,[,F])=>I+Number(F[o]||0),0),m=f-k,v=se(m),p=t?"text-blue-400":"text-violet-400",w="1fr 80px 80px 84px",T=`
    ...
    `,x=c.map(([I,F])=>{const A=Number(F[r]||0),H=Number(F[o]||0),P=A-H,O=se(P),K=P>0?"+":"",l=Ce(I),L=i=>i>0?i.toLocaleString("pt-BR",{minimumFractionDigits:2}):'<span class="text-on-surface-variant/30">—</span>';return`
    ...
    `;const[t,a]=await Promise.all([d.getAllDailyLogs(),d.getEmergencyFund()]);de=t||{},pe=a;const{diaBalance:n,dinheiroBalance:s}=Ae(de);e.innerHTML=He(n,s,a);const r=document.getElementById("fin-modals-host");r&&r.remove(),Pe()}const Re=["wb_sunny","fitness_center","coffee","restaurant","school","menu_book","restaurant_menu","edit_note","self_improvement","directions_run","bedtime","water_drop","favorite","psychology","music_note","code","brush","local_library","sports_soccer","hiking","spa","timer"];async function ye(){const e=document.getElementById("settings-root"),t=await d.getSettings(),a=[{name:"Verde (Default)",hex:"#72fe8f"},{name:"Azul",hex:"#4da6ff"},{name:"Roxo",hex:"#d48bff"},{name:"Vermelho",hex:"#ff5c5c"}];e.innerHTML=`
    ...
    `}function ue(){const e=window.APP_HABITS||[],t=Re.map(a=>`<button data-icon="${a}" onclick="window._habitIconPick('${a}', this)" class="habit-icon-opt w-10 h-10 rounded-xl border border-white/10 bg-surface-container flex items-center justify-center text-on-surface-variant hover:text-primary hover:border-primary/40 transition-all">
